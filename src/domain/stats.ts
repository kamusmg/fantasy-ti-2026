...
const def = (
  ...
  assumedCv: number,
  soloCredit = false,
): StatDefinition => ({ id, color, labelPtBr, rule, assumedCv, soloCredit });

export const STAT_DEFINITIONS: Readonly<Record<StatId, StatDefinition>> = {
  // ---------- VERMELHO ----------
  kills: def('kills', 'red', 'Abates', { kind: 'perUnit', pointsPerUnit: 107 }, 0.55),
  deaths: def('deaths', 'red', 'Poucas Mortes', { kind: 'baseMinus', base: 1950, perUnit: 195 }, 0.35),
  creepScore: def('creepScore', 'red', 'Creep Score', { kind: 'perUnit', pointsPerUnit: 3 }, 0.20),
  gpm: def('gpm', 'red', 'GPM', { kind: 'perUnit', pointsPerUnit: 2 }, 0.20),
  madstone: def('madstone', 'red', 'Madstones', { kind: 'perUnit', pointsPerUnit: 13 }, 0.40),
  towerKills: def('towerKills', 'red', 'Torres', { kind: 'perUnit', pointsPerUnit: 352 }, 0.70),

  // ---------- AZUL ----------
  wardsPlaced: def('wardsPlaced', 'blue', 'Wards Colocadas', { kind: 'perUnit', pointsPerUnit: 117 }, 0.25),
  campsStacked: def('campsStacked', 'blue', 'Acampamentos Empilhados', { kind: 'perUnit', pointsPerUnit: 234 }, 0.40),
  runes: def('runes', 'blue', 'Runas', { kind: 'perUnit', pointsPerUnit: 141 }, 0.35),
  watchers: def('watchers', 'blue', 'Vigias', { kind: 'perUnit', pointsPerUnit: 147 }, 0.45),
  smokes: def('smokes', 'blue', 'Smokes Usados', { kind: 'perUnit', pointsPerUnit: 293 }, 0.45),
  lotuses: def('lotuses', 'blue', 'Lotus', { kind: 'perUnit', pointsPerUnit: 176 }, 0.50),

  // ---------- VERDE ----------
  roshan: def('roshan', 'green', 'Roshan', { kind: 'perUnit', pointsPerUnit: 1172 }, 1.10, true),
  teamfight: def('teamfight', 'green', 'Participacao em Combate', { kind: 'capped', max: 2124 }, 0.15),
  stuns: def('stuns', 'green', 'Stuns', { kind: 'perUnit', pointsPerUnit: 10 }, 0.40),
  tormentor: def('tormentor', 'green', 'Tormentor', { kind: 'perUnit', pointsPerUnit: 879 }, 0.75),
  firstBlood: def('firstBlood', 'green', 'First Blood', { kind: 'flat', points: 1934 }, 2.00, true),
  courier: def('courier', 'green', 'Abates de Coruja', { kind: 'perUnit', pointsPerUnit: 703 }, 1.30, true),
} as const;
